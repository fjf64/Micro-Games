...
if True:
    ...
    areamap = {}
    walker_pos_list = []
    mapsize = [5, 20] #!!! IMPORTANT !!!#
    mapmid = [round(mapsize[0] / 2), round(mapsize[1] / 2)]
    oldpp = [3, 10] # INITIAL PLAYER POSITION
    origin = oldpp
    newpp = oldpp
    player = "X"
    blank = "#"
    ...
    def pathgen(path_length,path_stretch = 1):
        global walker_pos_list
        """creates randomized paths using [walkable] variable as the path, and path stretch is the linear distance covered before changing direction. """
        oldpp = origin
